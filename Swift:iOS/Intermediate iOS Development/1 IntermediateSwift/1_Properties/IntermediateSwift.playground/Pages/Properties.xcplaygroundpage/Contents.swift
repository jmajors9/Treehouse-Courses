...
class ReadItLaterNetworkingClient {
    ...
    // Do other things
    /* until we make a request, there is no value in the session var
     when we do, then it is assigned the value
     essentially a store property whose creation is deferred until needed.
     NSDateFormatter is a good example
     (But Swift 3 has a better version)
     */
}
